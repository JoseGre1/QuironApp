class StudentsController < ApplicationController
  ...
  def show
    if params[:id]
      ...
      @students = Institution.find_by(id: params[:institution_id]).students.order(global_score: :desc).first(params[:number].to_i)
      ...
      #Best Students
      @students = Student.order(global_score: :desc).first(params[:number].to_i)
    else
      #All Students
  ...
  private
    ...
    def student_params
      params.require(:student).permit(:name, :email, :icfes_score, :school_score, :extra_score, :age, :gender, :personal_id, :personal_id_type, :institution_id)
    end
